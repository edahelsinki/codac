...
def run_a3s(features, targetIds, q_max, arg_clus, arg_neighbor, arg_T, arg_tau, subsample=None, random_state=None):
    ...
    def test(gt_label, pred_label):
        ...
        # print("test:", count,nmi,ari,purity)
        #history.append((count,nmi,ari,purity))
        pred_record.append(np.copy(pred_label))
    ...
    def lead_element_small(key,count):
        ...
        if len(pile) < 2:
            return -1,count
        pile = ll_sort(pile)
        if compactness(key, tau=arg_tau):
            leader = pile[0]
            return leader,count
        else:
    ...
    def outlier_merge(count):
        ...
        for key in keys_list_1:
            ...
            if query(targetIds,s1,s2):
                ...
                for s in [s1,s2]:
                    idx = 1
                    nei = knn[s][idx]
                    while nei == s and idx < arg_neighbor:
                        idx += 1
                        nei = knn[s][idx]
        ...
        for key in keys_list_1:
            ...
            idx = 1
            nei = knn[item][idx]
            while nei == item and idx<arg_neighbor:
                idx += 1
                nei = knn[item][idx]
    ...
    def fast_prob_clustering(target_features=None, Prob_mat=None, types='vector', epsilon=0.0001, n_neighbors=50, iso_reg_path=None, iso_reg_model=None):
    ...
    def train_iso_reg(X, k, n_neighbors, seed=None):
        print('Training isotonic regression model.')
        # fit k-means to get pseudolabels for isotonic regression
        ...
        dist, knn = neigh.kneighbors(X)
        dist = -(dist**2)
        dist = dist.flatten()

        # get binary labels for isotonic regression (1 for same cluster, 0 for different clusters)
        binlabels = []
        for i in range(knn.shape[0]):
            for j in range(knn.shape[1]):
                idx = knn[i][j]
                if pseudolabels[i] == pseudolabels[idx]:
                    binlabels.append(1)
                else:
                    binlabels.append(0)
        ...
        model.fit(dist, binlabels)
        print("Finished training isotonic regression model.")
        return model
    ...
    # iso_reg_model with known k
    iso_reg_model = train_iso_reg(features, k, arg_neighbor, seed=random_state)

    # perform fast probabilistic clustering to get adaptive cluster number K
    print("Performing fast probabilistic clustering")
    pclus_label, log_prob_dict, knn, cluster_dict = fast_prob_clustering(target_features=features,iso_reg_path="",n_neighbors=arg_neighbor, iso_reg_model=iso_reg_model)
    ...
    # iso_reg_model = joblib.load(iso_paths[arg_iso])
    ...
    #     Prob_mat, targetIds = get_feature_similarity(arg_d, arg_n, arg_types)
    #     N = n_list[arg_n]
    #     k = len(np.unique(targetIds))
    ...
    #             target_dict[key].add(i)
    #     # perform fast probabilistic clustering to get adaptive cluster number K
    #     pclus_label, log_prob_dict, knn, cluster_dict = fast_prob_clustering(Prob_mat=Prob_mat,types='matrix',n_neighbors=arg_neighbor)
    ...
    # --------------------------------------------------------------------------------------
    # implement BMAAS
    labels_prev = np.copy(pclus_label)

    for t in range(arg_T):
        key_list = []
        for key in cluster_dict.keys():
            if len(cluster_dict[key])>1:
                key_list.append(key)
        ...
        while len(key2ent) and count < q_max:
            ...
            for items in key2ent:
                if key1 in items and len(cluster_dict[key1]) < 2:
                    ...
                    key2ent.remove(items)
            if count_record[-1] != count: #count % 3 == 0:
                test(targetIds, pclus_label)

        if np.all(labels_prev == pclus_label):
            print("No change in labels, stopping early (t={})".format(t))
            break
        else:
            labels_prev = np.copy(pclus_label)
    ...
    print("FINAL CLUSTER NUMBER:", len(np.unique(pclus_label)))
    ...
    # print(result)
    print("FINAL USED QUERIES:", count)
    final_ari = adjusted_rand_score(targetIds, pclus_label)
    print("FINAL ARI:", final_ari)
    return count_record, pred_record


class A3S:
    """
    Deng, X., Liu, J., Zhong, H., Feng, F., Shen, C., He, X., Ye, J., & Wang, Z. (2024).
    A3S: A General Active Clustering Method with Pairwise Constraints.
    Proceedings of the 41st International Conference on Machine Learning, 10488-10505.

    Code source: https://github.com/xiangtanshi/A3S
    """
    def __init__(self, max_queries=100, n_neighbors=50, T=1000, clus_method="fpc", tau=0.5, subsample=None, random_state=None):
    ...
    def fit(self, X, y):
        # Run the A3S algorithm with the provided data
        self.history["queries"], self.history["labels"] = run_a3s(X, y, self.max_queries, self.clus_method, self.n_neighbors, self.T, self.tau, self.subsample, self.random_state)
...
if __name__ == "__main__":
    ...
    from experiments.data import get_handwritten
    X, y = get_handwritten()
    #X, y = load_iris(return_X_y=True)
    model = A3S(random_state=42, T=1000, subsample=None, n_neighbors=50, tau=0.5)
    model.fit(X, y)
    print('done')